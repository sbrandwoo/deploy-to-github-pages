...
describe('Preparation', () => {
  ...
  describe('on master branch', () => {
    ...
    it('does not create branch directory', () => {
      ...
      expect(shell.mkdir).toBeCalledWith('-p', 'deploy-directory');
    });
  ...
  describe('on a non-master branch', () => {
    ...
    it('creates branch directory and target directory', () => {
      expect(shell.mkdir).not.toBeCalled();
      prepareDeployDirectory('deploy-directory', options);
      expect(shell.mkdir).toHaveBeenNthCalledWith(1, '-p', 'deploy-directory/branch');
      expect(shell.mkdir).toHaveBeenNthCalledWith(2, '-p', 'deploy-directory/branch/not-master');
    });
